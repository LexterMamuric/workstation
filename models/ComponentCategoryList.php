 ...
 * Page class
 */
class ComponentCategoryList extends ComponentCategory
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ComponentCategoryList";
    ...
    public $FormName = "fcomponent_categorylist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "componentcategorylist";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'component_category';
        $this->TableName = 'component_category';
        ...
        // Table object (component_category)
        if (!isset($GLOBALS["component_category"]) || get_class($GLOBALS["component_category"]) == PROJECT_NAMESPACE . "component_category") {
            $GLOBALS["component_category"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "componentcategoryadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "componentcategorydelete";
        $this->MultiUpdateUrl = "componentcategoryupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'component_category');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page => View page
                    ...
                    $result["view"] = $pageName == "componentcategoryview"; // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        $this->id->Visible = false;
        $this->ComponentCategory->setVisibility();
        ...
        if ($this->IsModal) {
            $this->FormName = "fcomponent_categorygrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->ComponentCategory->AdvancedSearch->toJson(), ","); // Field Component Category
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fcomponent_categorysrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->ComponentCategory->AdvancedSearch->SearchValue = @$filter["x_ComponentCategory"];
        $this->ComponentCategory->AdvancedSearch->SearchOperator = @$filter["z_ComponentCategory"];
        $this->ComponentCategory->AdvancedSearch->SearchCondition = @$filter["v_ComponentCategory"];
        $this->ComponentCategory->AdvancedSearch->SearchValue2 = @$filter["y_ComponentCategory"];
        $this->ComponentCategory->AdvancedSearch->SearchOperator2 = @$filter["w_ComponentCategory"];
        $this->ComponentCategory->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        global $Security;
        $where = "";
        $this->buildSearchSql($where, $this->ComponentCategory, $default, true); // Component Category
        ...
        if (!$default && $this->Command == "search") {
            $this->ComponentCategory->AdvancedSearch->save(); // Component Category
    ...
    public function showFilterList()
    {
        ...
        // Field Component Category
        $filter = $this->queryBuilderWhere("Component Category");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->ComponentCategory, false, true);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->ComponentCategory->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        if ($this->BasicSearch->Keyword != "") {
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->ComponentCategory;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->ComponentCategory->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    {
        $this->ComponentCategory->AdvancedSearch->unsetSession();
    }
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->ComponentCategory->AdvancedSearch->load();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->ComponentCategory); // Component Category
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->id->setSort("");
                $this->ComponentCategory->setSort("");
            }
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"component_category\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"component_category\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fcomponent_categorylist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fcomponent_categorylist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"component_category\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fcomponent_categorylist\"" .
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("Component Category", $this->createColumnOption("Component Category"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcomponent_categorysrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcomponent_categorysrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcomponent_categorylist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_component_category", "data-rowtype" => ROWTYPE_ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_component_category",
    ...
    protected function loadSearchValues()
    {
        ...
        // Component Category
        if ($this->ComponentCategory->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ComponentCategory->AdvancedSearch->SearchValue != "" || $this->ComponentCategory->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->ComponentCategory->setDbValue($row['Component Category']);
    }
    ...
    protected function newRow()
    {
        ...
        $row['Component Category'] = $this->ComponentCategory->DefaultValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            // Component Category
            $this->ComponentCategory->ViewValue = $this->ComponentCategory->CurrentValue;

            // Component Category
            $this->ComponentCategory->HrefValue = "";
            $this->ComponentCategory->TooltipValue = "";
        } elseif ($this->RowType == ROWTYPE_SEARCH) {
            // Component Category
            if ($this->ComponentCategory->UseFilter && !EmptyValue($this->ComponentCategory->AdvancedSearch->SearchValue)) {
                if (is_array($this->ComponentCategory->AdvancedSearch->SearchValue)) {
                    $this->ComponentCategory->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->ComponentCategory->AdvancedSearch->SearchValue);
                }
                $this->ComponentCategory->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->ComponentCategory->AdvancedSearch->SearchValue);
            }
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fcomponent_categorysrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
