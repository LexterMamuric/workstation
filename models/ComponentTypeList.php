 ...
 * Page class
 */
class ComponentTypeList extends ComponentType
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ComponentTypeList";
    ...
    public $FormName = "fcomponent_typelist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "componenttypelist";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'component_type';
        $this->TableName = 'component_type';
        ...
        // Table object (component_type)
        if (!isset($GLOBALS["component_type"]) || get_class($GLOBALS["component_type"]) == PROJECT_NAMESPACE . "component_type") {
            $GLOBALS["component_type"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "componenttypeadd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "componenttypedelete";
        $this->MultiUpdateUrl = "componenttypeupdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'component_type');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page => View page
                    ...
                    $result["view"] = $pageName == "componenttypeview"; // If View page, no primary button
                } else { // List page
    ...
    public function run()
    {
        ...
        $this->id->Visible = false;
        $this->ComponentType->setVisibility();
        ...
        if ($this->IsModal) {
            $this->FormName = "fcomponent_typegrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = "";
        $savedFilterList = "";
        $filterList = Concat($filterList, $this->ComponentType->AdvancedSearch->toJson(), ","); // Field Component Type
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fcomponent_typesrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        $this->ComponentType->AdvancedSearch->SearchValue = @$filter["x_ComponentType"];
        $this->ComponentType->AdvancedSearch->SearchOperator = @$filter["z_ComponentType"];
        $this->ComponentType->AdvancedSearch->SearchCondition = @$filter["v_ComponentType"];
        $this->ComponentType->AdvancedSearch->SearchValue2 = @$filter["y_ComponentType"];
        $this->ComponentType->AdvancedSearch->SearchOperator2 = @$filter["w_ComponentType"];
        $this->ComponentType->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        global $Security;
        $where = "";
        $this->buildSearchSql($where, $this->ComponentType, $default, true); // Component Type
        ...
        if (!$default && $this->Command == "search") {
            $this->ComponentType->AdvancedSearch->save(); // Component Type
    ...
    public function showFilterList()
    {
        ...
        // Field Component Type
        $filter = $this->queryBuilderWhere("Component Type");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->ComponentType, false, true);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->ComponentType->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->ComponentType->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    {
        $this->ComponentType->AdvancedSearch->unsetSession();
    }
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->ComponentType->AdvancedSearch->load();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->ComponentType); // Component Type
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"component_type\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"component_type\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fcomponent_typelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fcomponent_typelist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"component_type\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fcomponent_typelist\"" .
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("Component Type", $this->createColumnOption("Component Type"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcomponent_typesrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcomponent_typesrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcomponent_typelist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_component_type", "data-rowtype" => ROWTYPE_ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_component_type",
    ...
    protected function loadSearchValues()
    {
        ...
        // Component Type
        if ($this->ComponentType->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ComponentType->AdvancedSearch->SearchValue != "" || $this->ComponentType->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->ComponentType->setDbValue($row['Component Type']);
    }
    ...
    protected function newRow()
    {
        ...
        $row['Component Type'] = $this->ComponentType->DefaultValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            // Component Type
            $this->ComponentType->ViewValue = $this->ComponentType->CurrentValue;

            // Component Type
            $this->ComponentType->HrefValue = "";
            $this->ComponentType->TooltipValue = "";
        } elseif ($this->RowType == ROWTYPE_SEARCH) {
            // Component Type
            if ($this->ComponentType->UseFilter && !EmptyValue($this->ComponentType->AdvancedSearch->SearchValue)) {
                if (is_array($this->ComponentType->AdvancedSearch->SearchValue)) {
                    $this->ComponentType->AdvancedSearch->SearchValue = implode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->ComponentType->AdvancedSearch->SearchValue);
                }
                $this->ComponentType->EditValue = explode(Config("MULTIPLE_OPTION_SEPARATOR"), $this->ComponentType->AdvancedSearch->SearchValue);
            }
        }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fcomponent_typelist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fcomponent_typesrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
